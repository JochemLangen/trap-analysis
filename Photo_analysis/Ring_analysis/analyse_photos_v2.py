...
for i in range(no_theta_points):
    ...
    if len(low_diff_indices) == 0:
        avg_diff_trough_index = len(pixel_radials)-2*averaging_int_size
    else:
        avg_diff_trough_index = int((low_diff_indices[0]+avg_peak_index)*averaging_int_size)
    low_intensity_indices = np.argwhere(pixel_avg[avg_peak_index:]<darkness_limit)
    if len(low_intensity_indices) == 0:
        avg_trough_index = len(pixel_radials)-2*averaging_int_size
    else:
        avg_trough_index = int((low_intensity_indices[0]+avg_peak_index)*averaging_int_size)
    print(avg_diff_trough_index,avg_trough_index, 'hihi')
    ...
    log_low_diff_indices = np.argwhere(log_avg_diff[log_avg_peak_index:]<0)
    if len(log_low_diff_indices) == 0:
        log_avg_trough_index = (len(log_avg_diff[log_avg_peak_index:]))*averaging_int_size-2
        print(log_avg_trough_index)
    else:
        log_avg_trough_index = int(log_low_diff_indices[0]*averaging_int_size)
    ...
    plt.scatter(radial_avg[int(log_avg_peak_index/averaging_int_size)],log_avg_diff[int(log_avg_peak_index/averaging_int_size)],color='blue',label='log_avg_peak_index')
    plt.scatter(radial_avg[int(avg_peak_index/averaging_int_size)],log_avg_diff[int(avg_peak_index/averaging_int_size)],color='orange',label='avg_peak_index')
    plt.scatter(radial_avg[int(avg_diff_trough_index/averaging_int_size)],log_avg_diff[int(avg_diff_trough_index/averaging_int_size)],color='green',label='avg_diff_trough_index')
    plt.scatter(radial_avg[int(avg_trough_index/averaging_int_size)],log_avg_diff[int(avg_trough_index/averaging_int_size)],color='black',label='avg_trough_index')
    plt.legend()
    # plt.xscale('log')
    plt.show()
    ...
    plt.scatter(radial_avg[int(log_avg_peak_index/averaging_int_size)],pixel_avg[int(log_avg_peak_index/averaging_int_size)],color='blue',label='log_avg_peak_index')
    plt.scatter(radial_avg[int(avg_peak_index/averaging_int_size)],pixel_avg[int(avg_peak_index/averaging_int_size)],color='orange',label='avg_peak_index')
    plt.scatter(radial_avg[int(avg_diff_trough_index/averaging_int_size)],pixel_avg[int(avg_diff_trough_index/averaging_int_size)],color='green',label='avg_diff_trough_index')
    plt.scatter(radial_avg[int(avg_trough_index/averaging_int_size)],pixel_avg[int(avg_trough_index/averaging_int_size)],color='black',label='avg_trough_index')
    plt.legend()
    plt.show()
    ...
    # print("peak index avg",avg_peak_index)
    plt.scatter(radial_avg[int(log_avg_peak_index/averaging_int_size)],avg_diff[int(log_avg_peak_index/averaging_int_size)],color='blue',label='log_avg_peak_index')
    plt.scatter(radial_avg[int(avg_peak_index/averaging_int_size)],avg_diff[int(avg_peak_index/averaging_int_size)],color='orange',label='avg_peak_index')
    plt.scatter(radial_avg[int(avg_diff_trough_index/averaging_int_size)],avg_diff[int(avg_diff_trough_index/averaging_int_size)],color='green',label='avg_diff_trough_index')
    plt.scatter(radial_avg[int(avg_trough_index/averaging_int_size)],avg_diff[int(avg_trough_index/averaging_int_size)],color='black',label='avg_trough_index')
    plt.legend()
    plt.show()
    avg_trough_index = avg_diff_trough_index
    ...
    if avg_peak_index >= peak_size:
        ...
        peak_index = avg_peak_index-peak_size + np.argmax(pixel_diff)
        ...
        peak_pixels = pixel_radials[0:avg_peak_index+peak_size]
    ...
    if log_avg_peak_index >= peak_size:
        ...
        log_diff = (log_peak_pixels[:-1]-log_peak_pixels[1:])
        log_peak_index = log_avg_peak_index + np.argmax(log_diff)
    ...
    trough_pixels = pixel_radials[avg_trough_index:avg_trough_index+peak_size]
    ...
    plt.figure()
    plt.plot(r_radials,pixel_radials)
    plt.scatter(trough_r,trough_pixels)
    plt.plot(trough_r[:-1],tr_pixel_diff)
    plt.scatter(r_radials[trough_index],pixel_radials[trough_index],marker='x',color='black')
    plt.xlim([trough_r[0]-20,trough_r[-1]+20])
    ...
    log_trough_index = log_avg_trough_index-peak_size + np.argmin(log_tr_pixel_diff)
    ...
    plt.plot(r_radials[peak_index:trough_index+1],final_pixels, color='red',zorder=5)
    plt.plot(r_radials[log_peak_index:][:log_trough_index],log_final_pixels, color='green',zorder=7,alpha=0.5)
    plt.xlim([r_radials[0]-50,r_radials[0]+5])
