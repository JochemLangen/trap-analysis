...
# top_index = 12
# rootdir = 'D:\\Jochem\\Documents\\Uni\\Physics\\Level 4 Project\\New_col\\Flip_focus_ring_axicon\\New_set\\Dist_range'
# top_index = 3

# rootdir = 'D:\\Jochem\\Documents\\Uni\\Physics\\Level 4 Project\\New_col\\Flipped_ring_axicon\\Img_dist_range_2\\Adj_cam' #
# rootdir = 'D:\\Jochem\\Documents\\Uni\\Physics\\Level 4 Project\\New_col\\Flipped_ring_axicon\\Img_dist_range\\Adj_cam' #
# rootdir = 'D:\\Jochem\\Documents\\Uni\\Physics\\Level 4 Project\\New_col\\Flipped_ring_axicon\\Dist_range'#'\\Adj_cam' #
# top_index = 7
# rootdir = 'D:\\Jochem\\Documents\\Uni\\Physics\\Level 4 Project\\New_col\\Flipped_ring_axicon\\Dist_range_2\\Adj_cam' #
...
for j in range(dist_len):
    ...
    for i in range(no_theta_areas):
        ...
        if slice_plot == True:
            ...
            plt.xlim(xlim)
            # plt.yscale('log')
            # plt.ylim([np.amin(pixel_radials_slice[pixel_radials_slice > 0]),1])
            plt.ylim(ylim)
            ...
            plt.xlim(xlim)
            # inner_slice_pixels = slice_pixels[abs(slice_r) < 1]
            # ylim = [np.amin(inner_slice_pixels[inner_slice_pixels.nonzero()]),np.amax(inner_slice_pixels)+0.3]
            # plt.yscale('log')
            plt.ylim(ylim)
        ...
        if report_plot == True:
            ...
            # ax = fig.add_axes((1.38+offset_axes,0.85,0.35,0.35))
            ...
            # bins_std = np.sqrt(np.average(mids[50:-50]**2, weights=bin_values[50:-50]))
            # print(bins_std,std_array[i])
            ...
            # xlim = [-std_array[i]*2,std_array[i]*2]
            xsize = abs(bin_edges[1:][bin_values > min_ref][ymin])
            xlim = [-xsize,xsize]
            plt.xlim(xlim)
            plt.ylim(ylim)
            plt.yscale('log')
            # plt.yticks(np.linspace(ylim[0],5*(ylim[-1]//5),6)[:-1],fontsize=fontsize)
    ...
    if report_plot == True:
        break
    #The method below is not used. Here the intensity was normalised with the general 16-bit image saturation (or simply max possible) and scaled with the exposure time
    # act_peak_intensity = peak_intensity * max_I[j] / max_pixel_value / exp_ms_array[j] #Normalise it by the maximum pixel value and the exposure time in ms
    # act_b_array = b_array * max_I[j] / max_pixel_value / exp_ms_array[j] #Normalise it by the maximum pixel value and the exposure time in ms
    # if max_I_err[j] == np.inf: #The error of saturated values was set to inf previously to average out saturated values. This does not make sense for this purpose
    #     act_peak_intensity_err = np.sqrt((peak_intensity_err * max_I[j] / max_pixel_value)**2 + (act_peak_intensity*exp_ms_err)**2)/exp_ms_array[j]
    #     act_b_err_array = np.sqrt((b_err_array * max_I[j] / max_pixel_value)**2 + (act_b_array*exp_ms_err)**2)/exp_ms_array[j]
    # else:
    #     act_peak_intensity_err = act_peak_intensity * np.sqrt((peak_intensity_err/peak_intensity)**2 + (max_I_err[j]/max_I[j])**2 + (exp_ms_err/exp_ms_array[j])**2)
    #     act_b_err_array = act_b_array * np.sqrt((b_err_array/b_array)**2 + (max_I_err[j]/max_I[j])**2 + (exp_ms_err/exp_ms_array[j])**2)
    ...
    if image_results == True:
        ...
        #Could consider using an average value instead. Can also add an error to it
